...
make quality judgments simpler and more objective. Growing out a league
based on the Sony Aibo quadruped robots, the league has constantly evolved, as it moves ever
closer to playing by human rules, and currently features the Aldebaran Nao humanoid robots. In the past the hallmark of the league has been
a focus on individual skills, such as perception, localization, and motion, at the expense
of more team-oriented skills, such as positioning and passing. The league has begun to address
these skills with the creation of the Drop-in Challenge, where each competing team is comprised
of robots taken from different research teams. This new focus should force teams to work
...
The original league adopted a standard robot platform, the Sony Aibo, as a way to make the focus
...
competition ultimately about the quality of the developed software. Since all competing teams use the
...
made this development interesting, however, was that these behaviors could be acquired and optimized using machine
learning. 
...
problems that Aibos had, but also created significant new problems.
...
it is facing. Contrast that to the early years, when there were two uniquely-colored
...
but the league has begun to converge on a single walk engine, as numerous
...
by an opponent. Since these kicks are less powerful, robots using them are not
...
As more teams use the B-Human (or a rough equivalent)
...
is required to use. The packet contains a number of standardized data fields
...
in the packet, including where the robot is walking to (or, thinks it is walking to), where it is shooting
...
There are many challenges involved in making such drop-in games work, which
...
towards a more human-like position system. A theoretical advantage that robots
...
a robot on the sideline, which can observe the game and communicate with the field players. The
danger of such an addition, of course, is that the coach, who will have an
fairly good view of the field, could provide information that would solve
problems, for example, where the ball is, what side of the field the ball or robots
...
because of the shapes of the robot's feet, limitations on the movements
of their joints, differences in field conditions, etc. Nevertheless, robots are able to kick
the ball with some accuracy over short distances. The biggest limitation seems to
lie in the receiving robots. Even the best humans do not always make perfect passes,
but what they do quite well is to quickly adjust to imperfect passes. Humans can quickly
shift their feet, their motion path, or whatever else is necessary in order to receive and
...
system up and running, and forces the best teams to keep improving, but on the other
...
Ultimately, the Standard Platform League 
...
advancing humanoid robot soccer in increasingly realistic directions, including the field dimensions, the
number of robots on the field, and the setup of the field. In turn, the emphasis on software has
led the league to develop
and refine techniques in motion, vision, localization, communication, behavior, and coordination, but also to experiment with applying machine learning and optimization techniques to a range of tasks.
